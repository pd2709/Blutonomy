...
An attempt to work with 3D landmark measurements
Inspired by example here: https://github.com/borglab/gtsam/blob/develop/examples/PlanarSLAMExample.cpp
...
int main(int argc, char **argv)
{
    ...
    Pose3 prior(Rot3(), Point3(0.0, 0.0, 0.0)); // prior mean is at origin
    auto priorNoise = noiseModel::Diagonal::Sigmas(
        (Vector(6)<<0.3,0.3,0.3,0.1,0.1,0.1).finished());           // 30cm std on x,y, 0.1 rad on theta
    graph.addPrior(x1, prior, priorNoise); // add directly to graph

    // Add two odometry factors
    Pose3 odometry(Rot3(), Point3(2.0, 0.0, 0.0));
    // create a measurement for both factors (the same in this case)
    auto odometryNoise = noiseModel::Diagonal::Sigmas(
        (Vector(6)<<0.2,0.2,0.2,0.1,0.1,0.1).finished()); // 20cm std on pos, 0.1 rad on angle
    graph.emplace_shared<BetweenFactor<Pose3>>(x1, x2, odometry, odometryNoise);
    graph.emplace_shared<BetweenFactor<Pose3>>(x2, x3, odometry, odometryNoise);

    // Add Range-Bearing measurements to two different landmarks
    // create a noise model for the landmark measurements
    auto measurementNoise = noiseModel::Diagonal::Sigmas(
        Vector3(0.1, 0.1, 0.2)); // 0.1 rad std on bearing (all coords), 20cm on range
    ...
    // Rot3 bearing11 = Rot3::ypr(-M_PI/4, 0, 0), bearing21 = Rot3::ypr(-M_PI/2, 0, 0), bearing32 = Rot3::ypr(-M_PI/2, 0, 0);

    Unit3 bearing11(1,1,0);
    Unit3 bearing21(0,1,0);
    Unit3 bearing32(0,1,0);

    // Add Bearing-Range factors
    graph.emplace_shared<BearingRangeFactor<Pose3, Point3>>(x1, l1, bearing11, range11, measurementNoise);
    graph.emplace_shared<BearingRangeFactor<Pose3, Point3>>(x2, l1, bearing21, range21, measurementNoise);
    graph.emplace_shared<BearingRangeFactor<Pose3, Point3>>(x3, l2, bearing32, range32, measurementNoise);
    ...
    initialEstimate.insert(x1, Pose3(Rot3(), Point3(0.5, 0.0, 0.2)));
    initialEstimate.insert(x2, Pose3(Rot3(), Point3(2.3, 0.1, -0.2)));
    initialEstimate.insert(x3, Pose3(Rot3(), Point3(4.1, 0.1, 0.1)));
    initialEstimate.insert(l1, Point3(1.8, 2.1, 0));
    initialEstimate.insert(l2, Point3(4.1, 1.8, 0));
